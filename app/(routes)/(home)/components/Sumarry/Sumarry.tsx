...
import { SumarryProps } from "./Sumarry.types";
import Image from "next/image";
import { Button } from "@/components/ui/button";
import { Confetti } from "@/components/shared/Confetti";

export function Sumarry(props: SumarryProps) {
  const { onReload } = props;
  const { infoUser } = useStepConfig();
  const { avatarUrl, name, username, typeUser, platforms } = infoUser;
  return (
    <div>
      <h2 className="text-center font-semibold text-2xl">Your BPtree is ready</h2>
